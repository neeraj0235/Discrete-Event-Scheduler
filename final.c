...
bool cmp_burst(Node* a, Node* b) {
    if(a->arrival == b->arrival && a->remaining_burst == b->remaining_burst) {
        return strcmp(a->PID, b->PID) == -1;
    } else if(a->remaining_burst == b->remaining_burst) {
        return a->arrival < b->arrival;
    }
...
void print(Node** jobs, int N, char** exec_IDs, float exec_period[][2], int exec_ct) {
    ...
    printf("%s %.3f %.3f \n", current_job, start, end);
    // printf("PID %s \n", current_job);
    // printf("started %f \n", start);
    // printf("ended %f \n", end);

    // for(int i = 0; i < exec_ct; i++) {
    //     printf("%s %.3f %.3f ", exec_IDs[i], exec_period[i][0], exec_period[i][1]);
    // }
    // printf("\n");
    // printf("Avg turnaround time = %f\nAvg. response time = %f\n", turnaround/N, response/N);
    // printf("Turnaround time = %f\nResponse time = %f\n", turnaround, response);
    printf("%.3f %.3f", turnaround/N, response/N);
...
void rr_scheduling(Node** jobs, int N, float ts) {
    ...
    while(job_ct < N) {
        ...
        while(!isEmpty(q)) {
            ...
            float actual_burst = min(ts, current_job->remaining_burst);
    ...
    while(job_ct < N) {
        ...
        while(!isEmpty(q0) || !isEmpty(q1) || !isEmpty(q2)) {
            while(!isEmpty(q0)) {
                ...
                //     }
                // }
                Node* current_job = front(q0);

                if(current_job->first_schedule == -1) {
                    current_job->first_schedule = current_time;
                }

                float expected_burst = min(ts0, current_job->remaining_burst);
...
int main (int argc, char* argv[]) {
    if(freopen(argv[1], "r", stdin) == NULL) {
        perror("Couldn't find input file");
        return EXIT_FAILURE;
    }
    ...
    float TsMLFQ1 = atof(argv[4]), TsMLFQ2 = atof(argv[5]), TsMLFQ3 = atof(argv[6]), BMLFQ = atof(argv[7]);
